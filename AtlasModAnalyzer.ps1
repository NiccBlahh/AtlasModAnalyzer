...
if (-not (Test-Path -LiteralPath $strcanDirectory)) {
    Write-Host "  Invalid path. Could not find: $strcanDirectory" -ForegroundColor Red
    Start-Sleep -Seconds 3
    exit 1
}

try {
    $itemInfo = Get-Item -LiteralPath $strcanDirectory -Force -ErrorAction Stop
...
function Analyze-ArchiveContent {
    ...
    $weakDqrkisSigs = @('b.re', 'h Back', 'ayer:', '??tra', 'Assist', 'ChunkBorders', 'e Click', '?flower', 'guiKey', 'v2.1')
    $strongDqrkisSigs = @("?45", "?ea?", "2.4s", "A.n?", "A.p'", "aG'", "and'", "B.o'", "B.r'", "C.??", "D.am", "E.SP", "E.xp", "F.a", "jdwp", "O.n", "P.l", "R.an", "r-ca", "S.t", "S.to", "S.w", "Saved:", "U.??", "W.hi", "potionchime", "potionchime.config.effects", "potionchime.config.title", "potionchime.config.volume", "potionchime.config.warning_window")
    
    $hasStrong = $false
    foreach ($str in $itematchedStrs) {
        if ($strongDqrkisSigs -contains $str) { $hasStrong = $true; break }
    }
    
    if (-not $hasStrong) {
        foreach ($w in $weakDqrkisSigs) { [void]$itematchedStrs.Remove($w) }
    }
...
Write-Host " Credits to MeowModAnalyzer" -ForegroundColor DarkGray
